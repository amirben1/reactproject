...
class AuditReportGenerator:
    ...
    def create_header_footer(self, canvas, doc):
        ...
        company_name = self.data.get('client_name', 'Not specified')
    ...
    def create_cover_page(self, story, data):
        ...
        story.append(Paragraph("INTERNAL AUDIT REPORT", self.title_style))
        story.append(Paragraph("MANAGEMENT SYSTEM", self.subtitle_style))
        story.append(Paragraph(data.get('management_system', 'QUALITY').upper(), self.subtitle_style))
        
        story.append(Spacer(1, 40))
        
        # Client info in a styled table
        client_info = [
            ["CLIENT", data.get('client_name', 'Not specified')],
            ["ADDRESS", data.get('client_address', 'Not specified')],
            ["AUDIT PERIOD", data.get('audit_period', 'Not specified')],
            ["STANDARD", data.get('reference_standard', 'ISO 9001:2015')]
        ]
        ...
        report_types = [["☑  Internal Audit Report"]]
        ...
        date_text = f"Report Date: {current_date}"
        ...
        # Signature box
        signature_data = [
            ["Audit Manager: ", data.get('audit_manager', 'Not specified')],
        ]
    ...
    def create_toc(self, story):
        ...
        toc_data = [
            ...
            ["2. Audit Objective, Basis, and Scope", ""],
            ["3. Internal Audit Approach", ""],
            ["4. Compliance Evaluation Against the Standard", ""],
            ["5. Positive Points / Improvement Potential", ""],
            ["6. Conclusion", ""],
            ["7. General Remarks", ""]
        ]
    ...
    async def create_report(self, output_path, data):
        ...
        try:
            ...
            story.append(Paragraph("1. Summary Evaluation", self.header_style))
            story.append(Spacer(1, 15))
            
            # Client info in a modern table
            client_info = [
                ["Client", data.get('client_name', 'Not specified')],
                ["Audit Site", data.get('audit_site', 'Not specified')],
                ["Auditor", data.get('auditor_name', 'Not specified')],
                ["Standard", data.get('reference_standard', 'ISO 9001:2015')],
                ["Audit Type", data.get('audit_type', 'Internal Audit')],
                ["Audit Period", data.get('audit_period', 'Not specified')],
                ["Audit Manager", data.get('audit_manager', 'Not specified')],
                ["Audit Team", data.get('audit_team_members', 'Not specified')]
            ]
            ...
            summary_text = f"""As part of an internal audit of the company {data.get('client_name', 'Not specified')}, located at {data.get('client_address', 'Not specified')}, evidence has been provided that it has implemented and is improving a {data.get('management_system', 'quality')} management system that is partially compliant with the standards mentioned above."""
            story.append(Paragraph(summary_text, self.normal_style))
            story.append(Spacer(1, 12))
            
            # Non-conformities summary in a highlighted box
            nonconf_text = f"{data.get('non_conformities_count', '0')} Non-conformities in various chapters of the standard have been identified and documented in this report."
            ...
            current_date = datetime.now().strftime("%d.%m.%Y")
            signature_data = [
                [current_date, "", data.get('audit_manager', 'Not specified')],
                ["Date", "", "Audit Manager"]
            ]
            ...
            story.append(Paragraph("2. Audit Objective, Basis, and Scope", self.header_style))
            story.append(Spacer(1, 15))
            
            objective_text = f"The objective of the audit was to verify that the requirements of the standards related to the {data.get('management_system', 'quality')} management system are met and that operational conditions are fulfilled."
            story.append(Paragraph(objective_text, self.normal_style))
            story.append(Spacer(1, 15))
            
            # Reference documents in a styled box
            story.append(Paragraph("The following documents served as the basis for the audit:", self.normal_style))
            ...
            story.append(Paragraph("Scope of Application", self.subheader_style))
            domain_text = f"The audit pertains to the site located at {data.get('audit_site', 'Not specified')}, for the activity {data.get('activity_description', 'Not specified')}."
            story.append(Paragraph(domain_text, self.normal_style))
            story.append(Spacer(1, 10))
            
            # Process list in a styled box
            story.append(Paragraph("Included Processes:", self.normal_style))
            story.append(Spacer(1, 5))
            ...
            story.append(Paragraph("3. Internal Audit Approach", self.header_style))
            story.append(Spacer(1, 15))
            
            # Approach description in a highlighted box
            approach_text = "The audit was conducted through sampling, verifying process compliance through interviews and document review."
            ...
            story.append(Paragraph("4. Compliance Evaluation Against the Standard", self.header_style))
            story.append(Spacer(1, 15))
            
            # Modern compliance table with improved styling
            compliance_headers = [
                [Paragraph("Process", self.table_header_style),
                 Paragraph("Requirement", self.table_header_style),
            ...
            for idx, item in enumerate(data.get('compliance_items', [])):
                ...
                row = [
                    Paragraph(item.get('process', 'Not specified'), self.table_cell_style),
                    Paragraph(item.get('requirement', 'N/A'), self.table_cell_style),
                    Paragraph(item.get('comment', 'No information'), self.table_cell_style),
            ...
            story.append(Paragraph("5. Positive Points / Improvement Potential", self.header_style))
            story.append(Spacer(1, 15))
            
            # Positive points in a styled box
            story.append(Paragraph("Positive Points:", self.subheader_style))
            story.append(Spacer(1, 5))
            ...
            story.append(Paragraph("Improvement Potential:", self.subheader_style))
            story.append(Spacer(1, 5))

            recommendations = []
            # Use 'recommendations' key from the structured_data output
            for point in data.get('recommendations', []):
                recommendations.append([Paragraph(f"• {point}", self.bullet_style)])

            if recommendations:
                improvement_table = Table(recommendations, colWidths=[15*cm])
            ...
            conclusion_paragraphs = [
                f"No elements were identified during the audit that could challenge the following statements.",
                f"The audit objectives have been achieved.",
                f"The auditor identified {data.get('non_conformities_count', '0')} non-conformities across several chapters of the standards.",
                f"Compliance with the requirements of the standard {data.get('reference_standard', 'ISO 9001:2015')} has been verified and confirmed."
            ]
            ...
            story.append(Paragraph("7. General Remarks", self.header_style))
            story.append(Spacer(1, 15))
            remarks_text = data["resume"]
            story.append(Paragraph(remarks_text, self.normal_style))
            story.append(Spacer(1, 24))
            story.append(Paragraph("End of report.", self.normal_style))
